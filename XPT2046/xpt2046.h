 ...
 * Возможно подключение и использование внешнего источника опорного напряжения (см. схему своей платы,
 ...
 *
 * ВНИМАНИЕ! Если на вашей плате к 9 выводу микросхемы Vref подведено питание (как правило, накоротко
 * соединено с VCC - 1 вывод м/с), то при измерениях АЦП всегда в качестве опорного напряжения будет
 * использовать то напряжение, которое подведено в внешнему Vref. Поэтому в даташите указано, что
 * если планируется использование внутреннего источника опорного напряжения, то вывод Vref должен
 * оставаться неподключенным. То есть вывод Vref - это двунаправленный вывод, работающий как вход, если
 * подключен источник внешнего опорного напряжения и как выход внутреннего источника опорного напряжения,
 * если он включен соответствующим битом в управляющем байте. Эти тонкости не важны при использовании
 * тачскрина, но критичны, если XPT2046 используется для измерения температуры (датчиком внутри
 * самого чипа) и напряжений (Vbat и AUX). Неверная настройка управляющего байта и неправильное
 * указание значения опорного напряжения в таких случаях существенно исказят измерения.
 */
