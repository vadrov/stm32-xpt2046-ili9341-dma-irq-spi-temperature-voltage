...
- If your board has power supplied to pin 9 of the VREF chip (usually shorted to VCC - pin 1 of the I/O module, IOVDD - pin 10 of the I/O module, and VREF - pin 9 of the I/O module), the ADC will always use the voltage supplied to the external VREF source as the reference voltage for measurements. Therefore, the datasheet states that if the internal reference voltage is to be used, the VREF pin should be left unconnected. This means that the VREF pin is a bidirectional pin, functioning as an input if an external reference voltage source is connected, and as an output of the internal reference voltage source if enabled by the corresponding bit in the control byte. These subtleties aren't important when using a touchscreen, but they are critical if the XPT2046 is used to measure temperature (using a sensor inside the chip itself, essentially measuring the temperature of the chip itself, not the surrounding environment) and voltages (on the VBAT and AUX pins). Incorrectly setting the control byte and specifying the incorrect reference voltage value in such cases will significantly distort the measurements.
